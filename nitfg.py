...
class NITfg:
    ...
    def nitfg_test(self): 
        ...
        for epoch in range(self.n_epoch):
            ...
            score = cc_square(target1, target2)+0.0012  # set 0.001 to make type I error almost = 0.05
            if score > 0.01:
                break
            loss = -(score)
            loss.backward()    
...
def main_nitfg(d1,d2): 
    ...
    bar = 0.01
    ind = True
    score = NITfg(dataset=test_data)
    val = score.nitfg_test()
    if val > bar:
        ind = False 
    else:
        ind = True
