function [skeleton] = PC_sklearnTest(Tskeleton,data,maxCset,Alg)
n = size(data,2);
skeleton = ones(n,n);
for c = 1:n
    skeleton(c,c) = 0;
    ...
    for j = i+1:n
        ...
        for k = 0:maxCset
            if skeleton(i,j) == 1 && continFlag == 1
                if k == 0
                    [ppp,ind] = Alg(data(:,i),data(:,j),[]);
                    if ind
                        skeleton(i,j) = 0;
                        skeleton(j,i) = 0;
                        if Tskeleton(i,j) == 1 || Tskeleton(j,i) == 1
                           aaaaa = ppp
%                         else
%                             ccccc = ppp
                        end
                    ...
                    for t = 1:len_csetM
                        [ppp,ind] = Alg(data(:,i),data(:,j),data(:,csetM(t,:)));
                        if ind
                            skeleton(i,j) = 0;
                            skeleton(j,i) = 0;
                            if Tskeleton(i,j) == 1 || Tskeleton(j,i) == 1
